...
public class ShapesEater implements Shape,Animation{
	   double radius;
	   Point center;
	...
	public void play() {
	    ...
	    if(welt.getClosestShape(this)!=null){
	    	Shape closet = welt.getClosestShape(this);
	    if(closet.contains(center.x+radius,center.y+radius)||closet.contains(center.x+radius,center.y-radius)||
	    		closet.contains(center.x-radius,center.y+radius)||closet.contains(center.x-radius,center.y-radius)){
	    	this.color=closet.getColor();
	    	welt.removeShape(closet);
	    	this.radius += 1.5;
	    }}
